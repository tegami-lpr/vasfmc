...
FMCFCUStyleAM::FMCFCUStyleAM(ConfigWidgetProvider* config_widget_provider,
                           Config* main_config,
    ...
    m_last_range_left(-1), m_last_range_right(-1), m_last_mode_left(-1), m_last_mode_right(-1)    
{
    ...
    {
        Logger::log(QString("FMCFCUStyleAM: loaded input areas from %1").
    ...
    else
    {
        ...
        if (!m_input_area.saveToFile(m_fcu_config->getValue(CFG_FCU_INPUT_AREA_FILE)))
            Logger::log("FMCFCUStyleAM: could not save defauls FCU input areas");
        else
            Logger::log(QString("FMCFCUStyleAM: saved default FCU input areas to (%1)").
                        arg(m_fcu_config->getValue(CFG_FCU_INPUT_AREA_FILE)));
    }
...
void FMCFCUStyleAM::slotRefresh()
{ 
    QTime timer;
    timer.start();
...
#if VASFMC_GAUGE
void FMCFCUStyleAM::paintGauge(QPainter *pPainter)
{
...
void FMCFCUStyleAM::slotInputMouseButton(const QString& text, QMouseEvent* event)
{
...
void FMCFCUStyleAM::slotInputMouseWheel(const QString& text, QWheelEvent* event)
{
    MYASSERT(event != 0);
...
void FMCFCUStyleAM::keyPressEvent(QKeyEvent *event)
{
    MYASSERT(event != 0);
...
void FMCFCUStyleAM::mousePressEvent(QMouseEvent *event)
{
...
void FMCFCUStyleAM::mouseMoveEvent(QMouseEvent *event)
{
#if !VASFMC_GAUGE
...
void FMCFCUStyleAM::wheelEvent(QWheelEvent* event)
{
...
bool FMCFCUStyleAM::eventFilter(QObject * watched_object, QEvent* event)
{
#if !VASFMC_GAUGE
...
void FMCFCUStyleAM::paintMe(QPainter& painter)
{
    const FlightStatus* flightstatus = m_fmc_control->flightStatus();
...
void FMCFCUStyleAM::resetBackgroundImage()
{
#if VASFMC_GAUGE
...
void FMCFCUStyleAM::ReceiveMessage(FMCMessage *message) {
    FMCFCUStyleBase::ReceiveMessage(message);
}
